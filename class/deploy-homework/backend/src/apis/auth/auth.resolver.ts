...
export class AuthResolver {
  ...
  async login(
    ...
    this.authService.setRefreshToken({ user, res: context.res }); // context에 있는 res를 res로 담아서 보내준다.
    console.log(context);

    // 5. accessToken(=JWT)을 만들어서 프론트엔드(쿠키)에 보내주기
    return this.authService.getAccessToken({ user }); // => JWT , return해서 프론트한테 넘겨줌.
  }

  @UseGuards(GqlAuthRefreshGuard) // 여기서는 refreshToken 검증(refreshToken을 통과한 사람만 쓸 수 있게금!!)
  @Mutation(() => String)
  restoreAccessToken(
    @CurrentUser() currentUser: ICurrentUser, // 타입을 ICurrentUser로 정해줘서 타입을 설정해줬다.
  ) {
